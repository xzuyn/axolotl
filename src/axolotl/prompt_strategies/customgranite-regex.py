"""Module containing the CustomGranitePromptTokenizingStrategy class"""
...
class CustomGranitePromptTokenizingStrategy(PromptTokenizingStrategy):
    """
    Tokenizing strategy for CustomGranite.
    """
    ...
    def tokenize_prompt(self, prompt):
        ...
        for i, turn in enumerate(prompt[conversation_name]):
            ...
            prefix_text = ("\n" if i != 0 else "") + f"<|start_of_role|>{role_dict[turn['from']]}<|end_of_role|>"
            ...
            tokenized_text, regex_mask_labels = mask_regex_attention_tokenizer(
                ...
                text=f"{prefix_text}{ftfy.fix_text(sharegpt_value).strip()}<|end_of_text|>",
        ...
        for turn_segment in turn_segments:
            ...
            if current_length + turn_segment_length > self.sequence_len - (
                1 if self.tokenizer.add_bos_token and self.tokenizer.bos_token_id else 0
            ):
                break
            else:
...
# Function to load the CustomGranitePromptTokenizingStrategy
def load(tokenizer, cfg):
    return CustomGranitePromptTokenizingStrategy(None, tokenizer, cfg.train_on_inputs, cfg.sequence_len)
